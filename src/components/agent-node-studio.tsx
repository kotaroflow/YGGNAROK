...
export function AgentNodeStudio() {
  ...
  const projectedNodes = useMemo(() => {
    ...
    const cy = containerSize.height / 2 + 100;
  ...
  return (
    <div className="grid min-h-[calc(100vh-6rem)] gap-5 lg:grid-cols-[minmax(0,1fr)_auto] overflow-hidden">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-line bg-surface/30 shadow-sm flex flex-col">
        ...
        <div ref={canvasRef} className="relative h-[580px] overflow-hidden bg-black flex-grow flex items-center justify-center cursor-move">
          <Canvas camera={{ position: [0, 0, 6.5], fov: 45 }}>
            ...
            <directionalLight position={[-10, 5, -12]} intensity={4} color="#2dd4bf" />
            <directionalLight position={[10, -10, -12]} intensity={3} color="#f59e0b" />
          ...
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full border border-line bg-surface/80 text-foreground backdrop-blur-md shadow-lg transition hover:text-brand hover:border-brand/45"
      ...
      <aside 
        ...
        className="flex flex-col justify-between h-full overflow-y-auto overflow-x-hidden shrink-0 space-y-4"
      >
        <div className="w-[360px] space-y-4 pr-1">
          {/* Editor panel */}
