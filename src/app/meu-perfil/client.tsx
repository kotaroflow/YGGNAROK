...
export function MeuPerfilClient({
  ...
  const [cargo, setCargo] = useState("Líder de Estratégia");
  const [bio, setBio] = useState("Desenvolvedor e modelador neural no ecossistema YGGNAROK.");
  const [avatarColor, setAvatarColor] = useState("bg-brand"); // preset colors: bg-brand, bg-purple-500, bg-sky-500, bg-emerald-500
  ...
  return (
    <main className="min-h-screen text-foreground relative">
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-6 lg:px-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { id: "dark", label: "Void", desc: "Dark Mode", icon: Moon },
                        { id: "light", label: "Amber", desc: "Light Mode", icon: Sun }
                      ].map((item) => {
        ...
        <div className="mt-10 rounded-2xl border border-red-500/20 bg-red-500/5 p-6 shadow-xl backdrop-blur-md relative overflow-hidden">
          <div className="absolute right-0 top-0 size-32 bg-red-500/5 blur-2xl pointer-events-none" />
          <div className="flex items-start gap-3">
            <div className="size-9 rounded-xl bg-red-500/10 text-red-500 flex items-center justify-center shrink-0 border border-red-500/20">
              <ShieldAlert className="size-5" />
            </div>
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-foreground">Zona de Perigo Crítico</h3>
              <p className="text-[11px] text-muted leading-relaxed">
                Ações irreversíveis de sistema. Ao excluir a conta, todos os roteiros, LTM de agentes e histórico de campanhas serão removidos do Supabase permanentemente.
              </p>
              
              <button
                type="button"
                onClick={() => {
                  if (confirm("ATENÇÃO: Deseja REALMENTE excluir a sua conta? Esta ação NÃO PODE ser desfeita e todas as suas campanhas e memórias de IAs serão apagadas permanentemente.")) {
                    alert("A exclusão foi cancelada por segurança neste ambiente de demonstração.");
                  }
                }}
                className="mt-4 inline-flex items-center justify-center rounded-xl bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 text-xs font-bold transition duration-200 shadow-lg shadow-red-500/15"
              >
                Excluir Minha Conta
              </button>
            </div>
