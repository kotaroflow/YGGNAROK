    ...
    <AppShell>
      <main className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[380px_1fr] lg:px-8">
        <section className="rounded-lg border border-line bg-surface p-6 shadow-sm backdrop-blur ">
          ...
          <h1 className="mt-1 text-2xl font-bold text-foreground">Perfis</h1>
          <p className="mt-2 text-sm text-muted">Crie e organize os perfis operacionais do sistema.</p>
        ...
        <section className="rounded-lg border border-line bg-surface p-6 shadow-sm backdrop-blur ">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-bold text-foreground">Perfis disponiveis</h2>
              <p className="mt-1 text-sm text-muted">{profiles.length} perfil(is) visiveis nesta sessao.</p>
            </div>
          </div>
              ...
              <article key={profile.id} className="py-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-muted">{profile.description || "Sem descricao."}</p>
                  </div>
