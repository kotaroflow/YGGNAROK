          ...
          <form action={action} className="w-full max-w-md">
            <div className={`mb-9 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold shadow-sm ${
              variant === "cadastro"
                ...
                : "border-brand/20 bg-brand/10 text-brand dark:border-brand/20 dark:bg-brand/15 dark:text-brand"
            }`}>
              <ShieldCheck size={14} />
            ...
            {status ? <p className="mt-5 rounded-2xl bg-brand/10 p-3 text-sm text-brand dark:bg-brand/15 dark:text-brand">{status}</p> : null}
            ...
            <label className="mt-8 block text-sm font-medium text-slate-700 dark:text-stone-200">
              ...
              <span className="relative mt-2 block">
                ...
                <input name="email" type="email" required placeholder="name@email.com" className="h-12 w-full rounded-2xl border border-slate-200 bg-white/80 pl-11 pr-4 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-500 focus:border-brand/50 focus:ring-4 focus:ring-brand/15 dark:border-white/10 dark:bg-neutral-900/70 dark:text-stone-100 dark:focus:ring-brand/15" />
              </span>
            </label>
            ...
            <p className="mt-6 text-center text-sm text-slate-600 dark:text-stone-300">
              {footerLabel} <Link className="font-medium text-brand hover:text-brand-strong" href={footerHref}>{footerAction}</Link>
            </p>
          </form>
        </div>
