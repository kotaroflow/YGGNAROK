...
const TODAY_GLOW_AMBER = "0 0 14px rgba(245,158,11,0.4), 0 0 28px rgba(245,158,11,0.15)";
...
const TODAY_PULSE_AMBER = `
  @keyframes cal-today-pulse {
    0%, 100% { box-shadow: 0 0 10px rgba(245,158,11,0.3), 0 0 20px rgba(245,158,11,0.1); }
    50% { box-shadow: 0 0 18px rgba(245,158,11,0.45), 0 0 36px rgba(245,158,11,0.18); }
  }
`;
...
export function InteractiveCalendar({ initialContents }: { initialContents: ContentItem[] }) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 lg:px-8">
      ...
      <style>{isDark ? TODAY_PULSE_VOID : TODAY_PULSE_AMBER}</style>
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full bg-brand px-5 py-2.5 text-sm font-semibold text-neutral-950 transition hover:bg-brand-strong shadow-sm shadow-brand/20 shrink-0"
        >
          <Plus size={16} />
          Agendar Conteudo
        </button>
                  ...
                  <>
                    ...
                    <div className="relative flex items-center justify-between">
                      ...
                      <span
                        className={`relative inline-flex size-7 items-center justify-center rounded-lg text-xs font-bold transition-all duration-300
                          ${isToday
                            ? "bg-brand text-neutral-950 shadow-[0_0_12px_rgba(245,158,11,0.3)] dark:shadow-[0_0_12px_rgba(139,92,246,0.35)] scale-105 z-10"
                            : deliveryDay
                        ...
                        `}
                        style={isToday ? { boxShadow: isDark ? TODAY_GLOW_VOID : TODAY_GLOW_AMBER } : {}}
                      >
                        {day}
                      </span>
                      ...
                      {deliveryDay && !isToday && (
                        <span className="shrink-0 flex items-center justify-center size-4 rounded-full bg-amber-500 dark:bg-amber-400 shadow-[0_0_6px_rgba(245,158,11,0.4)]" title={deliveryDay.label}>
                    ...
                    <div className="relative mt-1.5 space-y-0.5 flex-1 overflow-y-auto max-h-[60px]">
                      {dayItems.slice(0, 3).map((item) => {
                        const statusColor =
                          item.status === "idea" ? "border-slate-500/20 text-slate-400 bg-slate-500/5" :
                    ...
                    {importantDay && !isToday && !deliveryDay && (
                      <div className="absolute bottom-0 left-2 right-2 h-0.5 rounded-full bg-gradient-to-r from-brand/30 via-brand/50 to-brand/30 dark:from-violet-500/30 dark:via-amber-500/50 dark:to-violet-500/30" />
                    )}
                  </>
                )}
              </div>
