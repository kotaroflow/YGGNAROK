      ...
      >
        ...
        {!open && (
          <div 
            ...
            style={{
              background: `linear-gradient(to right, color-mix(in srgb, var(--brand) 20%, transparent) 0%, color-mix(in srgb, var(--brand) 10%, transparent) 100%)`,
              width: `${usage.tokens > 0 ? Math.max(pct * 100, 5) : 0}%`,
              borderRight: usage.tokens > 0 ? '1px solid color-mix(in srgb, var(--brand) 30%, transparent)' : 'none',
              boxShadow: usage.tokens > 0 ? '2px 0 8px color-mix(in srgb, var(--brand) 15%, transparent)' : 'none'
            }}
          />
        )}
            ...
            <div className="mt-3 flex gap-1 rounded-lg bg-surface p-1 border border-line">
              <button
                ...
                className={`flex-grow rounded-md py-1.5 text-center text-[11px] font-bold transition duration-300 ${
                  filter === "recommended"
                    ? "bg-brand text-white dark:text-neutral-950 shadow-md transform scale-[1.02]"
