...
interface OpportunityItem { id: string; title: string; desc: string; impact: string; status: string; }
...
export function CommercialDashboardClient({ initialTab = "vendas" }: { initialTab?: string }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1360px] px-4 py-8 sm:px-6 lg:px-8 space-y-8 animate-fade-in text-foreground relative">
      ...
      {/* Toast Notification */}
      {toast && (
        <div className="fixed top-6 right-6 z-[var(--z-toast)] bg-surface border border-brand/40 text-foreground px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-alert-pop">
      ...
      {/* 1. Header */}
      <CommercialHeader onNovaCampanha={() => setShowNovaCampanha(true)} />
...
function CommercialHeader({ onNovaCampanha }: { onNovaCampanha: () => void }) {
  return (
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <button
            ...
            className="rounded-xl border border-line bg-surface-strong/30 hover:border-brand/30 hover:text-brand px-4 py-2.5 text-xs font-bold transition shadow-sm cursor-pointer"
          >
            Nova campanha
          </button>
          <Link
...
function MetricCard({ title, value, status, chip, icon: Icon }: MetricCardProps) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-line bg-surface/70 p-5 hover:border-brand/30 hover:shadow-[0_0_20px_rgba(248, 195, 102,0.05)] transition-all duration-300 flex flex-col justify-between min-h-[140px]">
      {/* Decorative Glow */}
...
function ReportsEmptyState({ showToast }: { showToast: (message: string, type?: "success" | "warning" | "info") => void }) {
  return (
    <div className="rounded-2xl border border-line bg-surface/40 p-8 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center md:items-start gap-6 backdrop-blur">
      <div className="size-16 rounded-2xl bg-brand/10 border border-brand/20 text-brand flex items-center justify-center shrink-0 shadow-[0_0_25px_rgba(248, 195, 102,0.08)]">
