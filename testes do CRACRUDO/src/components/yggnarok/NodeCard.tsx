...
export function NodeCard({
  ...
  // Scale independent hover/active ring adapting to theme brand color
  const glowClass = isSelected
    ? "ring-2 ring-brand shadow-[0_0_20px_rgba(var(--aura-color),0.28)]"
    : "hover:ring-1 hover:ring-brand/40 hover:shadow-[0_0_12px_rgba(var(--aura-color),0.14)]";

  return (
    <div
      className={clsx(
        "absolute rounded-xl border backdrop-blur-md select-none cursor-grab active:cursor-grabbing transition-all duration-300 hover:-translate-y-0.5",
        def.bg,
        def.border,
        glowClass,
