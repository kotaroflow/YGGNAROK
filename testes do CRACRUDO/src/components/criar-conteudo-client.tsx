...
export function CriarConteudoClient({ profiles, initialContents, activeTab: currentTab }: CriarConteudoClientProps) {
    ...
    if (connectionLineStyle === "orthogonal") {
      ...
      return (
        <polyline
          ...
          points={points}
          fill="none"
          stroke={isSelectedConnection ? "rgba(248, 195, 102,0.85)" : "rgba(248, 195, 102,0.35)"}
  ...
  return (
    <main className="min-h-screen text-foreground relative bg-[#080706] pb-10 select-none">
      <style>{`
        @keyframes subtleGlow {
          0% { border-color: rgba(248, 195, 102, 0.15); box-shadow: 0 0 10px rgba(248, 195, 102, 0.02); }
          50% { border-color: rgba(248, 195, 102, 0.35); box-shadow: 0 0 20px rgba(248, 195, 102, 0.08); }
          100% { border-color: rgba(248, 195, 102, 0.15); box-shadow: 0 0 10px rgba(248, 195, 102, 0.02); }
        }
        .glowing-panel {
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background: rgba(248, 195, 102, 0.2);
          border-radius: 99px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: rgba(248, 195, 102, 0.4);
        }
      `}</style>

      {/* Amber Aesthetic Backlight */}
      ...
      <div className="flex h-screen flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 relative">
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex flex-wrap items-center justify-between gap-3 bg-black/60 border border-line/30 rounded-xl p-3 backdrop-blur-md shadow-2xl select-none">
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-8 w-56 rounded-lg border border-line/40 bg-black/45 pl-8 pr-3 text-[10px] text-white outline-none focus:border-brand/40 transition font-semibold"
                ...
                <div className="flex items-center gap-1 border-l border-line/20 pl-2">
                  <button
                    ...
                    className={`h-8 px-2.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition ${
                      selectedNodeTypeFilter === "all" ? "bg-brand text-neutral-950" : "text-muted hover:text-white"
                    }`}
                  >
                    Todos
                  </button>
                  {Object.keys(CREATIVE_NODE_META).slice(0, 5).map(key => (
                    <button
                      ...
                      className={`h-8 px-2.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition ${
                        selectedNodeTypeFilter === key ? "bg-brand text-neutral-950" : "text-muted hover:text-white"
                      }`}
                    >
                ...
                <button
                  ...
                  className={`h-8 px-3 rounded-lg border text-[9.5px] font-black uppercase tracking-widest flex items-center gap-1.5 transition ${
                    showArchivedOnCanvas 
                      ...
                      : "border-line bg-black/20 text-muted/70 hover:text-white"
                  }`}
                >
                  <Archive size={11} />
            ...
            <div
              ref={canvasRef}
              className={`flex-1 relative overflow-hidden bg-[#0a090b] ${connectingFromId ? "cursor-alias" : "cursor-crosshair"}`}
              onMouseMove={handleCanvasMouseMove}
            >
              {/* Backlight layout */}
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(248, 195, 102,0.12),transparent_40%),linear-gradient(135deg,#0d0c0f_0%,#050406_100%)]" />
              <div className="pointer-events-none absolute inset-0 opacity-[0.14] [background-image:linear-gradient(rgba(248, 195, 102,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(248, 195, 102,0.08)_1px,transparent_1px)] [background-size:60px_60px]" />

              {/* Transform layout wrapper */}
              <div
                className="absolute inset-0 transition-transform duration-100 ease-out z-10"
                style={{
                  transform: `scale(${zoomLevel}) translate3d(var(--canvas-tx, 0px), var(--canvas-ty, 0px), 0)`,
                ...
                <svg className="absolute inset-0 z-0 size-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <marker id="creative-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
                      <path d="M 0 1 L 6 4 L 0 7 z" fill="rgba(248, 195, 102,0.65)" />
                    </marker>
                  </defs>
                  ...
                  return (
                    <div
                      ...
                      className={`absolute w-[240px] rounded-xl border p-3.5 shadow-2xl backdrop-blur-md select-none ${
                        ...
                        isSelected 
                          ? "ring-2 ring-brand/60 border-brand shadow-[0_0_40px_rgba(248, 195, 102,0.22)] scale-[1.01]" 
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute -right-1.5 -top-1.5 z-20 grid size-5 place-items-center rounded-full border border-line bg-black text-muted transition hover:border-red-500 hover:text-red-400"
                      ...
                      <div className="absolute -left-1.5 top-[46px] size-3 rounded-full border-2 border-black bg-[#100f13]" />
                      <button
                        onPointerDown={(e) => {
                          e.stopPropagation();
                        ...
                        className="absolute -right-2 top-[44px] grid size-4 cursor-crosshair place-items-center rounded-full border border-black bg-brand transition-transform hover:scale-125 hover:shadow-[0_0_8px_rgba(248, 195, 102,0.5)]"
                        title="Conectar"
                      >
              ...
              <div className="absolute bottom-4 left-4 flex gap-1 z-20 select-none">
                <button
                  ...
                  className="grid size-8 place-items-center bg-black/60 border border-line/20 rounded-lg text-muted hover:text-white backdrop-blur transition hover:bg-neutral-900"
                >
                  <ZoomIn size={12} />
                </button>
                <button
          ...
          <aside className="w-[360px] border-l border-line/25 bg-surface/15 backdrop-blur-xl flex flex-col shrink-0 overflow-y-auto custom-scrollbar select-text">
            {selectedCanvasNode ? (
              ...
              <div className="flex flex-col p-5 space-y-5 h-full">
                <div className="flex items-start justify-between gap-3 border-b border-line/20 pb-4">
                  <div className="min-w-0">
                    ...
                    <input
                      type="text"
                      className="mt-1 bg-transparent border-b border-transparent focus:border-brand/40 text-base font-black text-white outline-none w-full pb-0.5"
                ...
                <div className="space-y-5 flex-1">
                  ...
                  {selectedCanvasNode.type === "image" && (
                    <div className="space-y-3">
                      <div className="relative overflow-hidden rounded-xl border border-brand/20 bg-black/60 p-4 flex flex-col items-center justify-center h-44 shadow-inner">
                      ...
                      <div>
                        ...
                        <div className="flex rounded-lg border border-line/30 bg-black/40 p-2.5 items-start justify-between gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded bg-neutral-800 text-muted hover:text-white transition"
                            title="Copiar Prompt"
                          >
                            <Copy size={11} />
                          </button>
                  ...
                  {selectedCanvasNode.type === "video" && (
                    <div className="space-y-3">
                      <div className="relative overflow-hidden rounded-xl border border-rose-500/20 bg-rose-950/10 p-4 flex flex-col items-center justify-center h-44">
                      ...
                      <div>
                        ...
                        <textarea
                          rows={4}
                          className="w-full rounded-lg border border-line bg-black/40 p-3 text-[11px] font-medium leading-relaxed text-muted focus:border-brand focus:outline-none"
                  ...
                  {selectedCanvasNode.type === "chat" && (
                    <div className="space-y-3 flex flex-col h-[280px]">
                      ...
                      <div className="flex-1 overflow-y-auto border border-line/30 rounded-xl bg-black/35 p-3 space-y-2.5 custom-scrollbar min-h-0">
                        {(selectedCanvasNode.meta.messages || []).map((msg, i) => (
                          <div key={i} className={`flex flex-col ${msg.sender === "user" ? "items-end" : "items-start"}`}>
                            <div className={`max-w-[85%] rounded-xl px-3 py-2 text-xs font-semibold leading-relaxed shadow-sm ${
                              msg.sender === "user" 
                                ? "bg-brand text-neutral-950 rounded-br-none" 
                                : "bg-surface-strong border border-line/25 text-neutral-200 rounded-bl-none"
                            }`}>
                              {msg.text}
                            </div>
                      ...
                      <div className="flex gap-1.5 shrink-0">
                        <input
                          ...
                          className="flex-1 h-9 rounded-lg border border-line bg-black/40 px-3 text-xs text-white outline-none focus:border-brand/40 transition font-semibold"
                        ...
                        <button
                          ...
                          className="grid size-9 place-items-center rounded-lg bg-brand text-neutral-950 hover:bg-brand-strong transition shadow-[0_0_10px_rgba(248, 195, 102,0.2)]"
                        >
                          <Send size={12} />
                        </button>
                      </div>
                  ...
                  {selectedCanvasNode.type === "prompt" && (
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          type="text"
                          className="h-9 w-full rounded-lg border border-line bg-black/40 px-3 text-xs text-white outline-none focus:border-brand/40 font-semibold"
                      ...
                      <div>
                        ...
                        <textarea
                          rows={6}
                          className="w-full rounded-lg border border-line bg-black/40 p-3 text-[11px] font-mono leading-relaxed text-brand/90 focus:border-brand focus:outline-none"
                          value={selectedCanvasNode.content}
                          onChange={(e) => {
                            const val = e.target.value;
                            setCanvasNodes(prev => prev.map(n => n.id === selectedCanvasNode.id ? { ...n, content: val } : n));
                  ...
                  {selectedCanvasNode.type !== "prompt" && selectedCanvasNode.type !== "chat" && (
                    <div>
                      ...
                      <textarea
                        rows={4}
                        className="w-full rounded-lg border border-line bg-black/40 p-3 text-xs font-semibold leading-relaxed text-muted focus:border-brand focus:outline-none"
                        value={selectedCanvasNode.content}
                        onChange={(e) => {
                          const val = e.target.value;
                          setCanvasNodes(prev => prev.map(n => n.id === selectedCanvasNode.id ? { ...n, content: val } : n));
                      ...
                      <div className="grid gap-1.5">
                        {selectedCanvasConnections.map(node => (
                          <button
                            ...
                            className="flex items-center justify-between rounded-lg border border-line/25 bg-surface/10 hover:border-brand/35 hover:bg-surface/20 px-3 py-2 text-left text-xs font-semibold text-muted transition"
                          >
                        ...
                        <div className="space-y-2">
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              ...
                              <input
                                type="text"
                                className="h-8 w-full rounded border border-line bg-black/40 px-2 text-[10px] text-white font-semibold outline-none"
                                value={obsidianVault}
                      ...
                      {userRole === "admin" ? (
                        <div className="space-y-2">
                          <div>
                            ...
                            <input
                              type="text"
                              className="h-8 w-full rounded border border-line bg-black/40 px-2 text-[9px] text-white font-mono outline-none"
                              value={n8nUrl}
                ...
                <div className="grid gap-2 border-t border-line/20 pt-4 shrink-0">
                  {selectedCanvasNode.archived ? (
                    ...
                    <button
                      ...
                      className="flex h-9 items-center justify-center gap-1.5 rounded-lg border border-line bg-black/45 hover:bg-black/60 text-[10px] font-black uppercase tracking-wide text-muted hover:text-white transition"
                    >
                      <Archive size={11} />
                  ...
                  <button
                    ...
                    className="h-9 rounded-lg border border-line/20 bg-black/20 text-[10px] font-black uppercase text-muted hover:text-white transition"
                  >
                    Fechar Inspector
                  </button>
                </div>
              ...
              <div className="flex flex-col p-5 space-y-6 h-full">
                <div>
                  ...
                  <h2 className="text-base font-black text-white mt-1">Ingestão Visual</h2>
                ...
                <form onSubmit={handleInsertNode} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="h-9 w-full rounded-lg border border-line bg-black/40 px-3 text-xs text-white outline-none focus:border-brand/40 transition font-semibold"
                      value={newNodeTitle}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select
                        className="h-9 w-full rounded-lg border border-line bg-black/40 px-2 text-xs text-white font-semibold outline-none focus:border-brand/40 cursor-pointer"
                        value={newNodeType}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-lg border border-line bg-black/40 p-3 text-xs font-semibold text-white leading-relaxed outline-none focus:border-brand/40"
                  ...
                  <button
                    type="submit"
                    className="w-full flex h-10 items-center justify-center gap-2 rounded-lg bg-brand text-neutral-950 hover:bg-brand-strong transition font-black text-xs shadow-[0_0_15px_rgba(248, 195, 102,0.2)]"
                  >
                    <Plus size={14} />
                ...
                <div className="border-t border-line/25 pt-5 space-y-4 flex-1">
                  <div>
                    ...
                    <div className="flex rounded-lg border border-line/20 bg-black/20 p-1">
                      <button
                        type="button"
                        onClick={() => { setUserRole("standard"); showToast("Operador Standard ativado."); }}
                        className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-md text-center transition-all ${userRole === "standard" ? "bg-neutral-800 text-brand font-black" : "text-muted"}`}
                      >
                        Operador Standard
                      </button>
                      <button
                        ...
                        className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-md text-center transition-all ${userRole === "admin" ? "bg-brand text-neutral-950 font-black shadow-[0_0_10px_rgba(248, 195, 102,0.3)]" : "text-muted"}`}
                      >
                        Administrador
                      </button>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-black/40 rounded-xl p-3 border border-line/30 space-y-2 max-h-[150px] overflow-y-auto custom-scrollbar">
