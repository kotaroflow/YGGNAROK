...
export function IntegracoesClient() {
  ...
  return (
    <main className="min-h-screen text-foreground bg-background px-4 py-8 lg:px-8 relative">
      {/* Toast */}
      {toastMessage && (
        <div className="fixed top-6 right-6 z-[var(--z-toast)] bg-surface-overlay border border-line-strong text-foreground px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-alert-pop">
        ...
        <div className="space-y-4">
          {integrations.map((integration) => {
            ...
            return (
              <div
                key={integration.id}
                className="rounded-2xl border border-line bg-surface p-6 shadow-xl relative overflow-hidden backdrop-blur flex flex-col gap-6 transition hover:border-line-strong"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-xl bg-brand hover:bg-brand-strong text-white dark:text-neutral-950 px-4.5 py-2.5 text-xs font-bold transition shadow-sm self-start sm:self-auto shrink-0"
                  >
                    <Plus size={14} />
                    Conectar Conta
                  </button>
                  ...
                  <div className="space-y-2 border-t border-line/60 pt-4">
                    ...
                    <div className="grid gap-2">
                      {integration.accounts.map((acc) => (
                        <div
                          key={acc.id}
                          className="rounded-xl border border-line bg-surface-base/50 px-4 py-3 flex items-center justify-between gap-4 hover:border-line-strong transition"
                        >
                    ...
                    <div className="rounded-xl border border-line bg-surface-strong/30 p-5 space-y-4">
                      ...
                      <div className="grid gap-4 sm:grid-cols-3 items-end">
                        ...
                        <button
                          ...
                          className="h-10 flex items-center justify-center gap-1.5 rounded-xl bg-brand hover:bg-brand-strong text-white dark:text-neutral-950 text-xs font-bold transition shadow-sm w-full"
                        >
                          <Sparkles size={12} />
