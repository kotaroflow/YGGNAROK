import { hierarchySystemPrompt, orchestrationMetadata } from "@/lib/ai-hierarchy";
...
function withYggnarokHierarchy(messages: ChatMessage[], model: string) {
  const existingSystem = messages.find((message) => message.role === "system");
  const rest = messages.filter((message) => message !== existingSystem);
  const hierarchyContext = [
    hierarchySystemPrompt(),
    "No chat do site, opere como interface lucida do conselho: entenda o pensamento, ordene a tarefa, critique coerencia e entregue com proximo passo claro.",
    "Nao exponha chaves, variaveis secretas ou dados internos sensiveis.",
    `Metadata de orquestracao: ${JSON.stringify(orchestrationMetadata({ surface: "site_chat", model }))}`,
  ].join(" ");

  return [
    {
      role: "system" as const,
      content: existingSystem ? `${hierarchyContext}\n\nContexto original do sistema:\n${existingSystem.content}` : hierarchyContext,
    },
    ...rest,
  ];
}
...
export async function POST(req: Request) {
  ...
  const model = pickModel(clientModel);
  const routedMessages = withYggnarokHierarchy(finalMessages, model);
