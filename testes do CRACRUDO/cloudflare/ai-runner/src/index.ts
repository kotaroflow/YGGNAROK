...
const YGGNAROK_AI_ARCHITECTURE = "YGGNAROK_LUCID_MULTI_AGENT_HIERARCHY";
const YGGNAROK_ADMIN_AUTHORITY = {
  key: "adm_master",
...
const hierarchyPrompt = [
  ...
  "O usuario administrador sempre tem autoridade maxima sobre aprovacao, rejeicao, retrabalho e publicacao.",
...
const agentKeys = ["hefesto", "gaia", "daikokuten", "sarutahiko", "wenchang", "heimdall", "omoikane", "isis"] as const;
...
const agentPrompts: Record<AgentKey, { module: string; system: string }> = {
  ...
  gaia: { module: "growth", system: "Voce e Gaia, agente de monetizacao. Sugira valor, canais, riscos, objetivos e proximas acoes comerciais. Responda JSON." },
  daikokuten: { module: "campaigns", system: "Voce e Daikokuten, agente de campanhas. Estruture oferta, campanha, afiliados, links, riscos e oportunidades. Nao gaste nem publique automaticamente. Responda JSON." },
  sarutahiko: { module: "posting", system: "Voce e Sarutahiko, agente de postagem manual. Gere checklist, legenda para copiar, hashtags e pendencias. Nao prometa autopost. Responda JSON." },
  wenchang: { module: "library", system: "Voce e Wenchang, agente de biblioteca. Classifique referencias, prompts e materiais reutilizaveis. Responda JSON." },
  heimdall: { module: "system", system: "Voce e Heimdall, agente tecnico e roteador. Analise jobs, logs, saude e seguranca sem expor segredos. Responda JSON." },
  omoikane: { module: "reports", system: "Voce e Omoikane, agente de relatorios. Gere metricas interpretadas, resumo e recomendacoes. Responda JSON." },
...
async function runOpenAiCompatible(env: Env, system: string, payload: Record<string, unknown>, provider: string) {
  ...
  const candidates = await Promise.all(executorRoles.map(async (role) => {
    ...
    const output = await callOpenRouter(env, provider, model, [
      { role: "system", content: `${hierarchyPrompt} ${system} Voce e o executor ${role}. Dominio: ${domain}. Modo: ${mode}. Responda apenas JSON com {"summary":string,"items":array,"next_actions":array,"risk":string,"metadata":object}.` },
  ...
  const critiques = await Promise.all(criticRoles.map(async (role) => {
    const model = modelFor(env, role);
    const output = await callOpenRouter(env, provider, model, [
      { role: "system", content: `${hierarchyPrompt} Voce e o critico ${role} do YGGNAROK. Compare candidatos, encontre falhas e preserve qualidade. Responda apenas JSON com {"summary":string,"items":array,"next_actions":array,"risk":string,"metadata":object}.` },
  ...
  const final = (mode === "fast" || freeFallbackOnly) && !critiques.length ? candidates[0]?.output : await callOpenRouter(env, provider, modelFor(env, "consolidator"), [
    { role: "system", content: `${hierarchyPrompt} ${system} Voce e o consolidador final. Use o melhor dos candidatos e das criticas, resolva conflitos e entregue saida pronta. Responda apenas JSON com {"summary":string,"items":array,"next_actions":array,"risk":string,"metadata":object}.` },
...
function pickAgent(type: string, payload: Record<string, unknown> | null): AgentKey {
  ...
  if (value.includes("report") || value.includes("relatorio")) return "omoikane";
  ...
  if (value.includes("library") || value.includes("biblioteca") || value.includes("prompt")) return "wenchang";
